...
const webPlayerDir = path.resolve(projectRoot, "web_player");
...
console.log("=================================================");
console.log("📸 GENERADOR DE CAPTURAS PARA STORES (PLAYWRIGHT)");
console.log(`📁 Carpeta destino: ${outputDir}`);
console.log("=================================================");

async function main() {
  // 1. Iniciar servidor Vite local
  ...
  // --- 1. GOOGLE PLAY PHONE (1080 x 1920) ---
  console.log("📸 [1/7] Generando capturas para Google Play Teléfono (1080x1920)...");
  {
  ...
  // --- 2. GOOGLE PLAY TV / HORIZONTAL (1920 x 1080) ---
  console.log("📸 [2/7] Generando capturas para Android TV y Horizontal (1920x1080)...");
  {
    const context = await browser.newContext({
    ...
    // Clic en canal para capturar el reproductor
    try {
      const channelCard = page.locator(".channel-card, .channel-item, button").first();
      if (await channelCard.isVisible()) {
        await channelCard.click();
        await page.waitForTimeout(1000);
        await page.screenshot({ path: path.join(outputDir, "04_google_play_android_tv_player.png") });
      }
    } catch {
      // Si no hace clic, toma la pantalla completa
      await page.screenshot({ path: path.join(outputDir, "04_google_play_android_tv_player.png") });
    }
    await context.close();
  }
  ...
  console.log("📸 [3/7] Generando capturas para Tablet 10 pulgadas (2560x1600)...");
  {
    const context = await browser.newContext({
  ...
  console.log("📸 [4/7] Generando capturas para iPhone 6.7 pulgadas (1290x2796)...");
  {
    const context = await browser.newContext({
  ...
  // --- 5. APPLE APP STORE - IPAD PRO 12.9" (2048 x 2732) ---
  console.log("📸 [5/7] Generando capturas para iPad Pro 12.9 pulgadas (2048x2732)...");
  {
    const context = await browser.newContext({
  ...
  // --- 6. FEATURE GRAPHIC / BANNER GOOGLE PLAY (1024 x 500) ---
  console.log("📸 [6/7] Generando Gráfico de Funciones / Banner Google Play (1024x500)...");
  {
    const context = await browser.newContext({
  ...
  console.log("📸 [7/7] Generando Icono de la App (512x512)...");
  {
    ...
    const brandEl = page.locator(".brand-mark, .nexora-brand, svg, h1").first();
  ...
  console.log("=================================================");
  console.log("✅ ¡TODAS LAS CAPTURAS FUERON GENERADAS CON ÉXITO!");
  console.log(`📁 Revisa la carpeta en tu escritorio: ${outputDir}`);
  console.log("=================================================");
}
